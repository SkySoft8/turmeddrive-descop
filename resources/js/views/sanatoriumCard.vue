                                ...
                                >
                                    ...
                                    <div v-for="subcat in items" class="subcategory">
                                        ...
                                        <div v-for="product in products" class="service">
                                            <div class="d-flex col justify-content-between"
                                                 v-if="product.sanatorium_todo_item_id === subcat.id && subcat.sanatorium_todo_list_id === list.id && list.user_id === card.user_id && product.deleted_at === null ">
                                                <p>{{ product.title }}</p>
                                                <p>{{ product.price }}</p>
                                            </div>
                                        </div>
                        ...
                        <div class="col col-12 col-md-6">
                            ...
                            <div class="reviews">
                                <div v-for="comment in comments" class="reviews-item">
                                    <div v-if="comment.sanatoriumCard_id === card.id">
                              ...
                              state.user === comment.user_name) ?
                               ...
                               'disabled' ">Удалить</a>
                                ...
                                <div :class="state.user !== '' ? '' : 'hide'">
                                    ...
                                    <p><input class="comment-button" @click.prevent="commentSanatorium" type="submit"
                                              value="Отправить"></p>
                                </div>
                            </div>
                        </div>
...
export default {
    name: "sanatoriumCard",
    components: {ModalSanatorium, swiper,FsLightbox},
    setup() {
        const {state} = user;
        return {state};
    },
    ...
    methods: {
        getCard() {
            this.axios.get('/api/sanatorium/' + this.$route.params.id)
                .then(res => {
                    this.card = res.data.data;
                    let center = [this.card.coordinate_l, this.card.coordinate_r];
                    let item = res.data.data.images
                    const imgs = []
                    item.forEach( (key) => {                    
        ...
        getAction() {
            this.axios.get('/api/sanatoriumAction')
                .then(res => {
                    this.actions = res.data.data;
                })
        },
        getServic() {
            this.axios.get('/api/sanatoriumServic')
                .then(res => {
        ...
        getCatagoties() {
            this.axios.get('/api/sanatoriumCategories')
                .then(res => {
                    this.categories = res.data.data;
                })
        },
        commentSanatorium() {
            this.axios.post(`/api/sanatorium/${this.card.id}/comments`, {
                'message': this.message,
                'sanatoriumCard_id': this.card.id,
        ...
        getComment(page = 1) {
            this.axios.post('/api/sanatoriumShow/' + this.$route.params.id + '/comments', {
                'page': page
            })
                .then(res => {
        ...
        deleteComment(id) {
            this.axios.delete(`/api/sanatorium/comment/${id}`)
                .then(res => {
                    this.getComment()
                })
        },
        getList() {
            this.axios.get('/api/sanatoriumLists')
                .then(res => {
        ...
        getItem() {
            this.axios.get('/api/sanatoriumItems')
                .then(res => {
                    this.items = res.data.data;
                })
        },
        getProduct() {
            this.axios.get('/api/sanatoriumProducts')
                .then(res => {
