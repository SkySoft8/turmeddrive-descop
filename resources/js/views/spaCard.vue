                                    ...
                                    <div v-for="list in lists" class="category fadeInUp wow animated"
                                        ...
                                        <div v-for="subcat in items" class="subcategory">
                                            <div class="d-flex col justify-content-between">
                                                ...
                                                <div><label
                                                    v-if="subcat.spa_todo_list_id === list.id && list.user_id === card.user_id && subcat.deleted_at === null">{{ subcat.price }}</label>
                                                </div>
                                            </div>

                                            <div v-for="product in products" class="service">
                                                <div class="d-flex col justify-content-between"
                                                     v-if="product.spa_todo_item_id === subcat.id && subcat.spa_todo_list_id === list.id && list.user_id === card.user_id && product.deleted_at === null ">
                                                    <p>{{ product.title }}</p>
                                                    <p>{{ product.price }}</p>
                                                </div>
                                            </div>
                            ...
                            <div class="col col-12 col-md-6 hotels-inf-item">
                                ...
                                <div class="reviews">
                                    <div v-for="comment in comments" class="reviews-item">
                                        <div v-if="comment.spaCard_id === card.id">
                              ...
                              state.user === comment.user_name) ?
                               ...
                               'disabled' ">Удалить</a>
                                    ...
                                    <div :class="state.user !== '' ? '' : 'hide'">
                                        ...
                                        <p><input class="comment-button" @click.prevent="commentSpa" type="submit"
                                                  value="Отправить"></p>
                                    </div>
                                </div>
...
export default {
    ...
    methods: {
        ...
        commentSpa() {
            this.axios.post(`/api/spa/${this.card.id}/comments`, {
                'message': this.message,
                'spaCard_id': this.card.id,
        ...
        getComment(page = 1) {
            this.axios.post('/api/spa/show/' + this.$route.params.id + '/comments', {
                'page': page
            })
                .then(res => {
        ...
        deleteComment(id) {
            this.axios.delete(`/api/spa/comment/${id}`)
                .then(res => {
                    this.getComment()
                })
        },
        getAction() {
            this.axios.get('/api/spaAction')
                .then(res => {
