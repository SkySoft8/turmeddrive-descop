    ...
    <div class="content-wrapper">
        ...
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0">Редактирование карточки туристического оператора</h1>
                    </div><!-- /.col -->
                            ...
                            <select name="district_id" id="country" class="form-control input-lg dynamic" data-dependent="state">
                                @foreach($districts as $district)
                                    <option value="{{$district->id}}"
                                            {{$district->id == $turoperatorCard->district_id ? ' selected' : '' }}>{{$district->title}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group w-50">
                            ...
                            <select name="republic_id" id="state" class="form-control input-lg dynamic" data-dependent="city">
                                @foreach($republics as $republic)
                                    <option value="{{$republic->id}}"
                                            {{$republic->id == $turoperatorCard->republic_id ? ' selected' : '' }}>{{$republic->title}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group w-50">
                            ...
                            <select name="city_id" id="city" class="form-control">
                                @foreach($cities as $city)
                                    <option value="{{$city->id}}"
                                            {{$city->id == $turoperatorCard->city_id ? ' selected' : '' }}>{{$city->title}}</option>
                                @endforeach
                            </select>
                        </div>
                            <div class="form-group w-50">
