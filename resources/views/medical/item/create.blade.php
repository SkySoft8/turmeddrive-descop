...
<div class="content-wrapper">
    ...
    <section class="content">
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-12">
                    <form action="{{route('medical.item.store')}}" method="POST" class="w-25">
                        @csrf
                        ...
                        <div class="form-group w-50">
                            ...
                            <select class="select2" name="medical_product_ids[]" multiple="multiple"
                                    data-placeholder="Выберите категорию" style="width: 100%;">
                                @foreach($medicalProducts as $medicalProduct)
                                    <option
                                            {{is_array(old('$medicalProduct_ids')) && in_array($medicalProduct->id, old('medicalProduct_ids')) ? ' selected' : ''}}
                                            value="{{$medicalProduct->id}}">{{$medicalProduct->title}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group w-50">
