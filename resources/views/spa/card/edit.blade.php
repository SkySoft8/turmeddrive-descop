    ...
    <div class="content-wrapper">
        ...
        <section class="content">
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-12">
                        <form action="{{route('spa.card.update', $spaCard->id)}}" method="POST"
                            ...
                            <div class="form-group">
                                <textarea class="textarea" rows='5' cols='70' name="desc">{{$spaCard->desc}}</textarea>
                                <div class="counter">
                            ...
                            <div class="form-group">
                                <textarea class="textarea1"   rows='10' cols='70' name="content">{{$spaCard->content}}</textarea>
                                <div class="counter1">
                            ...
                            <div class="form-group w-50">
                                ...
                                <div class="mb-3 d-flex">
                                    <img class="img-thumbnail" style="height: 30vh;"
                                         src="{{ url('storage/' . $spaCard->preview_image)}}" alt="preview_image">
                                </div>
                            ...
                            <div class="form-group w-50">
                                ...
                                <div class="mb-3 d-flex">
                                    <img class="img-thumbnail" style="height: 30vh;"
                                         src="{{ url('storage/' . $spaCard->logo)}}" alt="logo">
                                </div>
                                <div class="input-group">
                                    <div class="custom-file">
                            ...
                            <div class="form-group w-25">
                                <input type="text" class="form-control" name="name_link"
                                       placeholder="Обновить название ссылки на Вашу организацию"
                                       value="{{$spaCard->name_link}}">
                                @error('message')
                                <div class="text-danger">{{$message}}</div>
                                @enderror
                            </div>
                            <div class="form-group w-25">
                                <input type="text" class="form-control" name="link"
                                       placeholder="Обновить ссылку на Вашу организацию"
                                       value="{{$spaCard->link}}">
                                @error('phone')
                                <div class="text-danger">{{$message}}</div>
                                @enderror
                            ...
                            <div class="form-group w-25">
                                <input type="text" class="form-control" name="coordinate_l"
                                       placeholder="Введите координаты до запятой"
                                       value="{{$spaCard->coordinate_l}}">
                                @error('coordinate')
                                <div class="text-danger">{{$message}}</div>
                                @enderror
                            </div>
                            <div class="form-group w-25">
                                <input type="text" class="form-control" name="coordinate_r"
                                       placeholder="Введите координаты после запятой"
                                       value="{{$spaCard->coordinate_r}}">
                                @error('coordinate')
                                <div class="text-danger">{{$message}}</div>
                                @enderror
                            </div>
                            ...
                            <select name="district_id" id="country" class="form-control input-lg dynamic" data-dependent="state">
                                @foreach($districts as $district)
                                    <option value="{{$district->id}}"
                                            {{$district->id == $spaCard->district_id ? ' selected' : '' }}>{{$district->title}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group w-50">
                            ...
                            <select name="republic_id" id="state" class="form-control input-lg dynamic" data-dependent="city">
                                @foreach($republics as $republic)
                                    <option value="{{$republic->id}}"
                                            {{$republic->id == $spaCard->republic_id ? ' selected' : '' }}>{{$republic->title}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group w-50">
                            ...
                            <select name="city_id" id="city" class="form-control">
                                @foreach($cities as $city)
                                    <option value="{{$city->id}}"
                                            {{$city->id == $spaCard->city_id ? ' selected' : '' }}>{{$city->title}}</option>
                                @endforeach
                            </select>
                        </div>
                            <div class="form-group w-50">
                                ...
                                <select class="select2" name="spa_tag_ids[]" multiple="multiple"
                                    ...
                                    @foreach($spaTags as $spaTag)
                                        <option
                                                {{is_array( $spaCard->spaTags->pluck('id')->toArray()) && in_array($spaTag->id, $spaCard->spaTags->pluck('id')->toArray()) ? ' selected' : ''}}
                                                value="{{$spaTag->id}}">{{$spaTag->title}}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
